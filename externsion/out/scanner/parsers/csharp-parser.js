...
class CSharpParser {
    language = 'csharp';
    // C# modifiers
    static MODIFIERS = '(?:public|private|protected|internal|static|readonly|const|volatile|virtual|override|abstract|sealed|new|extern|unsafe|partial|async)';
    // Common C# types (including nullable types)
    static TYPE_PATTERN = '(?:[A-Z]\\w*(?:<[^>]+>)?(?:\\[\\])*\\??|int\\??|long\\??|short\\??|byte\\??|float\\??|double\\??|decimal\\??|bool\\??|char\\??|string\\??|void|object\\??|dynamic\\??|var)';
    parse(sourceCode, filePath) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const methodMatch = trimmedLine.match(new RegExp(`(?:${CSharpParser.MODIFIERS}\\s+)*(?:${CSharpParser.TYPE_PATTERN}\\s+)(\\w+)\\s*\\(([^)]*)\\)`));
            ...
            if (currentClass && !currentMethod) {
                ...
                if (propertyMatch) {
                    fields.push(this.createFieldDeclaration(propertyMatch.name, propertyMatch.type, filePath, lineNumber, line, lines, `class:${currentClass}`));
                    continue;
                }
    ...
    matchPropertyDeclaration(line) {
        // Pattern for property declarations: [modifiers] Type Name { get; set; }
        // Also handles expression-bodied properties: Type Name => value;
        const propertyPattern = new RegExp(`^(?:${CSharpParser.MODIFIERS}\\s+)*(${CSharpParser.TYPE_PATTERN})\\s+(\\w+)\\s*(?:\\{|=>)`);
    ...
    matchFieldDeclaration(line) {
        ...
        const fieldPattern = new RegExp(`^(?:${CSharpParser.MODIFIERS}\\s+)*(${CSharpParser.TYPE_PATTERN})\\s+(\\w+(?:\\s*=\\s*[^,;]+)?(?:\\s*,\\s*\\w+(?:\\s*=\\s*[^,;]+)?)*)\\s*;`);
    ...
    matchLocalVariable(line) {
        ...
        const localVarPattern = new RegExp(`^(?:const\\s+)?(${CSharpParser.TYPE_PATTERN})\\s+(\\w+)(?:\\s*=|;)`);
    ...
    extractCSharpComments(lines, lineNumber) {
        const comments = [];
