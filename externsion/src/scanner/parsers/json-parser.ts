...
export class JsonParser implements ILanguageParser {
  ...
  parse(sourceCode: string, filePath: string): ParseResult {
    ...
    try {
      ...
      const err = error as SyntaxError & { message: string };
  ...
  private extractFieldsWithLocations(
    ...
    const parseString = (): { value: string; startLine: number; startCol: number; endLine: number; endCol: number } => {
    ...
    const parseValue = (): void => {
      ...
      if (pos >= sourceCode.length) return;
      ...
      while (pos < sourceCode.length && sourceCode[pos] !== '}') {
        ...
        if (sourceCode[pos] === '"') {
          ...
          fields.push({
            ...
            context: {
              surroundingCode: extractSurroundingCode(lines, keyInfo.startLine),
   ...
   */
  private getJsonType(value: unknown): string | null {
