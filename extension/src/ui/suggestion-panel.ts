...
export class SuggestionPanel {
  ...
  getGroupedSuggestions(): GroupedSuggestions[] {
    ...
    const groups = new Map<MaskingPatternType, Suggestion[]>();
    ...
    return [...suggestions].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'location':
          ...
          if (fileCompare !== 0) return fileCompare * multiplier;
   ...
   */
  getSortOption(): { sortBy: SortOption; sortDirection: SortDirection } {
    return {
   ...
   */
  accept(suggestionId: string, applyToSimilar: boolean = false): void {
    const decision: UserDecision = { action: 'accept', applyToSimilar };
  ...
  reject(suggestionId: string, notes?: string): void {
    const decision: UserDecision = { action: 'reject', notes };
  ...
  defer(suggestionId: string): void {
    const decision: UserDecision = { action: 'defer' };
 ...
 */
export function createSuggestionPanel(config?: Partial<PanelConfig>): SuggestionPanel {
