...
/**
 * Represents a warning generated during configuration validation.
 */
export interface ValidationWarning {
  /** The setting key that had an invalid value */
  setting: string;
  /** Human-readable description of the validation issue */
  message: string;
  /** The invalid value that was provided */
  invalidValue: unknown;
  /** The fallback value that will be used instead */
...
export interface ValidationResult {
  ...
  isValid: boolean;
  /** List of warnings for invalid or problematic values */
  warnings: ValidationWarning[];
}

/** Valid severity values for diagnostics */
const VALID_SEVERITIES = ['Error', 'Warning', 'Information', 'Hint'] as const;
...
export class ConfigurationValidator {
  ...
  validateSeverity(severity: unknown): ValidationResult {
    ...
    if (!VALID_SEVERITIES.includes(severity as SeverityValue)) {
      warnings.push({
        setting: 'severity',
   ...
   */
  private normalizeForComparison(pattern: string): string {
    return pattern
