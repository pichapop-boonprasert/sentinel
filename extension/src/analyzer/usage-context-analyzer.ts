...
import { CodeLocation, FieldDeclaration, UsageContext } from '../types';
 ...
 */
export const HIGH_RISK_CONTEXT_TYPES: UsageContext['type'][] = ['logging', 'serialization', 'api_response'];

/**
 * Medium-risk context types
 */
export const MEDIUM_RISK_CONTEXT_TYPES: UsageContext['type'][] = ['storage', 'display'];
 ...
 */
export class UsageContextAnalyzer {
  ...
  analyzeField(field: FieldDeclaration): UsageContextAnalysisResult {
    ...
    const hasHighRiskContext = contexts.some(ctx => 
      HIGH_RISK_CONTEXT_TYPES.includes(ctx.type) || ctx.risk === 'high'
    );
   ...
   */
  private determineHighestRisk(contexts: UsageContext[]): UsageContext['risk'] {
    if (contexts.length === 0) {
      return 'low';
    }

    const riskOrder: UsageContext['risk'][] = ['high', 'medium', 'low'];
 ...
 */
export function createUsageContextAnalyzer(customPatterns: ContextPattern[] = []): UsageContextAnalyzer {
 ...
 * @returns Analysis result with detected contexts
 */
export function analyzeUsageContext(field: FieldDeclaration): UsageContextAnalysisResult {
 ...
 * @returns true if used in logging, serialization, or API response contexts
 */
export function isHighRiskUsage(field: FieldDeclaration): boolean {
  const analyzer = new UsageContextAnalyzer();
