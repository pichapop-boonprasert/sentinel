...
export class JavaParser implements ILanguageParser {
  ...
  // Java modifiers
  private static readonly MODIFIERS = '(?:public|private|protected|static|final|volatile|transient|abstract|synchronized|native|strictfp)';
  
  // Common Java types
  private static readonly TYPE_PATTERN = '(?:[A-Z]\\w*(?:<[^>]+>)?(?:\\[\\])*|int|long|short|byte|float|double|boolean|char|void)';
  ...
  parse(sourceCode: string, filePath: string): ParseResult {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const methodMatch = trimmedLine.match(
        new RegExp(`(?:${JavaParser.MODIFIERS}\\s+)*(?:${JavaParser.TYPE_PATTERN}\\s+)(\\w+)\\s*\\(([^)]*)\\)`)
      );
  ...
  private matchFieldDeclaration(line: string): { name: string; type: string }[] | null {
    ...
    const fieldPattern = new RegExp(
      `^(?:${JavaParser.MODIFIERS}\\s+)*(${JavaParser.TYPE_PATTERN})\\s+(\\w+(?:\\s*=\\s*[^,;]+)?(?:\\s*,\\s*\\w+(?:\\s*=\\s*[^,;]+)?)*)\\s*;?$`
    );
  ...
  private matchLocalVariable(line: string): { name: string; type: string }[] | null {
    ...
    const localVarPattern = new RegExp(
      `^(?:final\\s+)?(${JavaParser.TYPE_PATTERN}|var)\\s+(\\w+)(?:\\s*=|;)`
    );
  ...
  private extractJavaComments(lines: string[], lineNumber: number): string[] {
